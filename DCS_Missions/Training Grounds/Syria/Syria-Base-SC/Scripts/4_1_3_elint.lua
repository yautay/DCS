if (elint) then
    local elint_elements = {
        e1 = {"ELINT RC135", AIRBASE.Syria.Larnaca, zoneBlueElint11:GetCoordinate(), UTILS.FeetToMeters(35000), UTILS.KnotsToMps(250), zoneBlueElint12:GetCoordinate()},
        e2 = {"ELINT C130", AIRBASE.Syria.Ramat_David, zoneBlueElint21:GetCoordinate(), UTILS.FeetToMeters(20000), UTILS.KnotsToMps(250), zoneBlueElint22:GetCoordinate()}
    }
    local function init_elint_elements(templates_table)
        for k, v in pairs(templates_table) do
            -- template, airbase, coord, alt, speed, racetrack 
            SPAWN:New(v[1])
            :SpawnAtAirbase(AIRBASE:FindByName(v[2]))
            :InitLimit(1, 0)
            :SpawnScheduled(5, .1)
            :OnSpawnGroup(
                function(element_spawned)
                    element_spawned:TaskOrbit(v[3], v[4], v[5], v[6])
                    local unit_name = element_spawned:GetFirstUnitAlive():GetName()
                    env.info(string.format("SPAWNED %s\n", unit_name))
                    HoundBlue:addPlatform(unit_name)
                end):InitRepeatOnLanding()
        end
    end
    ...
    init_elint_elements({elint_elements.e1, elint_elements.e2})
